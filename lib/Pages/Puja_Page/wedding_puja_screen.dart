...
class WeddingPujaScreen extends ConsumerWidget {
  const WeddingPujaScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // 🚀 Listen to the filtered 'wedding' pujas
    final pujasState = ref.watch(weddingPujasProvider);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            const Text('Wedding Pujas', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18, letterSpacing: 0.2)),
            Text('Auspicious beginnings by expert Pandits', style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w500)),
          ],
        ),
      ...
      // 🔄 BULLETPROOF PULL-TO-REFRESH (Now in App Theme)
      body: RefreshIndicator(
        color: Colors.white,
        backgroundColor: Colors.deepOrange, // 🔥 Changed to App Theme
        strokeWidth: 3,
        displacement: 20,
        onRefresh: () async {
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: Colors.orange.shade50, shape: BoxShape.circle), child: const Icon(Icons.diversity_1, size: 80, color: Colors.deepOrange)), // 🔥 Changed to App Theme
          const SizedBox(height: 24),
          const Text("No Wedding Pujas", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.black87)),
          const SizedBox(height: 8),
          const Padding(padding: EdgeInsets.symmetric(horizontal: 40), child: Text("We couldn't find any wedding related pujas right now.", textAlign: TextAlign.center, style: TextStyle(color: Colors.grey, fontSize: 14, height: 1.4))),
        ],
      ),
    );
  }
